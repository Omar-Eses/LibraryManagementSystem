...
{
    // TODO : Change the lambda
    public async Task<User> AddUserAsync(User user, List<long>? permissionIds)
    {
        context.Users.Add(user);
        await context.SaveChangesAsync();

        if (permissionIds != null) await AssignPermissionsToUser(user.Id, permissionIds);
        // TODO : Handle the enum
        else await AssignPermissionsToUser(user.Id, new List<long> { 1, 2, 3, 4, 5, 6 });
